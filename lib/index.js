...
class IndentifyStream extends Transform {
   ...
   * @param {(Format|Format[])} [formats] - One or more custom format to check for.
   */
  constructor(options = {}) {
    super({
    ...
    // How much of a sample do we need to test every file type?
  ...
  _transform(chunk, encoding, cb) {
    if (this.complete === false) {
      ...
      if (this.sample.length >= this.sampleLength) {
        this.emit('complete', lookup(this.sample, this.customFormats));
        this.complete = true;
      }
    }
